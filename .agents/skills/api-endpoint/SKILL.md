---
name: api-endpoint
description: Create a backend API endpoint with schema, service, router, and test. Use when adding REST endpoints, CRUD routes, or backend APIs.
---

# API Endpoint

Build the standard backend slice for one endpoint family: schema, service, router, and test.
Use this for new REST resources or route additions, not for frontend-only work or database-only changes.
